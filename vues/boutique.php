    ...
    <main class="page-main">
        ...
        <div class="montres-grid">
            <?php if (empty($montres)): ?>
                <p style="color:#555; grid-column:1/-1;">Aucune montre trouvée.</p>
            <?php endif; ?>

            <?php foreach ($montres as $m): ?>
                <div class="montre-card">
                    <a href="detail_montre.php?id=<?php echo $m['id']; ?>">
                        <?php if (!empty($m['image'])): ?>
                            <img class="card-img" src="../assets/img/montres/<?php echo htmlspecialchars($m['image']); ?>" alt="">
                        <?php else: ?>
                            <div class="card-placeholder">⌚</div>
                        <?php endif; ?>
                    </a>
                    <div class="card-body">
                        <div class="card-marque"><?php echo htmlspecialchars($m['marque']); ?></div>
                        <div class="card-titre"><?php echo htmlspecialchars($m['titre']); ?></div>
                        <div class="card-prix"><?php echo number_format($m['prix'], 2, ',', ' '); ?> €</div>
                        <div class="card-actions">
                            <?php if (isset($_SESSION["id"])): ?>
                                <form method="POST" action="../admin/admin_panier.php">
                                    <input type="hidden" name="action" value="ajouter">
                                    <input type="hidden" name="montre_id" value="<?php echo $m['id']; ?>">
                                    <input type="hidden" name="retour" value="boutique">
